...
import Data.List.Extra (headDef, mapAccumL, intercalate)
...
unboxInteger a = do
    ...
    where
        ub (NInt i) = i
        ub x = error $ "Unboxing a non-integer! Got: " ++ show x
...
-- runners
run :: [(Identifier, Int, Code)] -> [GState]
run prog = evalState eval (GState initialCode [] [] heap globals)
    where
